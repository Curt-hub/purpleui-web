      ...
      >
        <div
          style={{
            ...
            height: 5,
            borderRadius: 9999,
            background: dark ? 'rgba(255,255,255,0.15)' : '#DDDDDF',   // suspected drift — see report (matches colors.loaderTrack but semantically a drag handle, not a loader)
          }}
        />
      </div>
      ...
      >
        <h2
          style={{
            ...
            fontWeight: 700,
            fontSize: 16,
            color: dark ? '#ffffff' : colors.onBackground,   // onBackground dark (no token) : onBackground
            margin: 0,
          }}
        >
          {title ?? ''}
        ...
        <button
          ...
          style={{
            ...
            height: 40,
            borderRadius: 9999,
            border: `1.5px solid ${colors.outlineSubtle}`,   // outlineSubtle
            background: dark ? 'transparent' : colors.background,   // background
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            cursor: 'pointer',
        ...
        >
          ...
          <svg width="18" height="17" viewBox="0 0 18 16.875" fill="none" aria-hidden="true">
            <path
              ...
              fill={dark ? '#ffffff' : colors.brand}   // brand (dark variant is a plain white icon, no token)
            />
          </svg>
        </button>
      </div>
